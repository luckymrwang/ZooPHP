...
class Manage extends MY_Controller {
	...
	public function del_server(){
		...
		$viewData['msg'] = $this->input->get_post('msg');
		$ret = $this->Model_app->getById($appid);
		$viewData['init_servers'] = explode(',',$ret['newservers']);

		if(empty($submit) or empty($zid_selected)){
			$this->render('accountant/manage/view_del_server', $viewData);
			return ;
		}
		
		foreach($zid_selected as $zid){
			$key = array_search($zid,$viewData['init_servers']);
			unset($viewData['init_servers'][$key]);
		}
		
		$all_ids = get_sub_app_ids($appid);
		if(!in_array($appid, $all_ids))
			$all_ids[] = $appid;
		
		foreach($all_ids as $sim_id) {
				$data['appid'] = $sim_id;
				$data['servers'] = $viewData['init_servers'];
				$data['newservers'] = $viewData['init_servers'];
				$ret = $this->Model_app->update($data);
		}

		$viewData['msg'] = $ret ? '1' : '0';
		//$this->layout_data['server_string'] = $newservers;
		redirect("accountant/manage/del_server?msg=$viewData[msg]&big_app_id=$appid");
	}
	
	public function get_new_servers($big_app_id){
		$ret = $this->Model_app->getById($big_app_id);
		$data = explode(',',$ret['newservers']);
		$html_string = '已开服 : ';
		foreach($data as $info){
			$html_string.="<input type='checkbox' name='zid_selected[]' value='$info' onmouseover='hideerror()'> $info ";
		}
		echo $html_string;
	}
